...
fn main() {
    ...
    root.left.clone().unwrap().borrow_mut().left =
        Some(Rc::new(RefCell::new(TreeNode::new("d".to_string()))));
    root.right.clone().unwrap().borrow_mut().left =
        Some(Rc::new(RefCell::new(TreeNode::new("e".to_string()))));
    root.right.clone().unwrap().borrow_mut().right =
        Some(Rc::new(RefCell::new(TreeNode::new("f".to_string()))));
    println!(
        "{}{:}{}",
        "The longest route will pass through ",
        longest_route(Some(Rc::new(RefCell::new(root)))),
        " checkpoints"
    );
}
