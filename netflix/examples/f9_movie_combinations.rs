...
fn backtrack(
    ...
    genres: Vec<String>,
    mut combinations: &mut Vec<String>,
) {
    // If the path is the same size() as genres, we have a complet combination
    ...
    movies.insert(
        "Family".to_string(),
        vec!["Frozen", "Kung fu Panda", "Ice Age"]
            .into_iter()
            .map(String::from)
            .collect(),
    );
    movies.insert(
        "Action".to_string(),
        vec!["Iron Man", "Wonder Woman", "Avengers"]
            .into_iter()
            .map(String::from)
            .collect(),
    );
    ...
    movies.insert(
        "Horror".to_string(),
        vec!["Oculus", "Sinister", "Insidious", "Annebelle"]
            .into_iter()
            .map(String::from)
            .collect(),
    );
...
fn letter_combinations(genres: Vec<String>) -> Vec<String> {
    ...
    // Initiate backtracking with an emrty path and starting index of 0
...
fn main() {
    ...
    // Example - 2;
    let genres2: Vec<String> = vec!["Family", "Action"]
        .into_iter()
        .map(String::from)
        .collect();
    ...
    // // Example - 3
    let genres3: Vec<String> = vec!["Horror", "Comedy"]
        .into_iter()
        .map(String::from)
        .collect();
    ...
    // // // Example - 4
    let genres4: Vec<String> = vec!["Horror", "Fantasy", "Comedy", "Family"]
        .into_iter()
        .map(String::from)
        .collect();
