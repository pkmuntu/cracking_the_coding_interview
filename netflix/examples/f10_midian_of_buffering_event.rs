...
fn median_sliding_window(nums: Vec<i32>, k: i32) -> Vec<f64> {
    ...
    loop {
        ...
        if (k & 1) == 1 {
            ...
            let s = (*small_list.peek().unwrap().0 as f64
                + *large_list.peek_min().unwrap().0 as f64)
                * 0.5;
            medians.push(s);
        }
        ...
        let out_num = nums[(i - k) as usize];
        let in_num = nums[i as usize];
        i += 1;
        ...
        // re-balance heap
        if balance < 0 {
            // `small_list` needs more valid elements
            small_list.push(
                *large_list.peek_min().unwrap().0,
                *large_list.peek_min().unwrap().0,
            );
            large_list.pop_min();
            balance += 1;
        ...
        if hashmap.contains_key(small_list.peek().unwrap().0) {
            while !hashmap.get(small_list.peek().unwrap().0).is_none()
                && hashmap.get(small_list.peek().unwrap().0).unwrap() > &0
            {
        ...
        if !large_list.is_empty() {
            if hashmap.contains_key(large_list.peek_min().unwrap().0) {
                while !large_list.is_empty()
                    && *hashmap.get(large_list.peek_min().unwrap().0).unwrap() > 0
                {
