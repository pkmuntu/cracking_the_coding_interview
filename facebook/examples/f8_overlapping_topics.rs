    ...
    while right < sifted_list_a.len() {
        ...
        while left <= right && unique_characters == count_unique_b {
            ...
            *unique_topics.get_mut(&topic).unwrap() -= 1;
            if dictlist_b.contains_key(&topic)
                && unique_topics.get(&topic).unwrap() < dictlist_b.get(&topic).unwrap()
            {
                unique_characters -= 1;
            }
...
fn main() {
    ...
    let a: Vec<String> = vec![
        "corona", "petrol", "corona", "corona", "climate", "petrol", "corona", "petrol",
    ]
    .into_iter()
    .map(String::from)
    .collect();
    let b: Vec<String> = vec!["corona", "petrol", "climate"]
        .into_iter()
        .map(String::from)
        .collect();
