...
fn create_tree_from_val(
    preorder: Vec<String>,
    left: i32,
    right: i32,
    inorder_val_idx: HashMap<String, i32>,
    mut preorder_idx: &mut i32,
    ...
    *preorder_idx += 1;
    ...
    root.left = create_tree_from_val(
        preorder.to_vec(),
        left,
        inorder_val_idx.clone().get(&rootVal).unwrap() - 1,
        inorder_val_idx.clone(),
        &mut preorder_idx,
    );
    root.right = create_tree_from_val(
        preorder.to_vec(),
        inorder_val_idx.clone().get(&rootVal).unwrap() + 1,
        right,
        inorder_val_idx.clone(),
...
fn re_creating_decision_tree(
    ...
    for i in 0..inorder.len() {
        inorder_val_idx
            .entry(inorder[i].to_string())
            .or_insert(i as i32);
    }

    return create_tree_from_val(
        preorder.to_vec(),
        0,
...
fn main() {
    // Driver code
    let preorder: Vec<String> = vec!["subject", "viewed", "notviewed", "similar", "nonsimilar"]
        .into_iter()
        .map(String::from)
        .collect();
    let inorder: Vec<String> = vec!["viewed", "subject", "similar", "notviewed", "nonsimilar"]
        .into_iter()
        .map(String::from)
        .collect();
