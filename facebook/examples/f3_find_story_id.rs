fn search(arr: Vec<i32>, start: i32, end: i32, key: i32) -> i32 {
    ...
    if arr[start as usize] <= arr[mid as usize]
        && key <= arr[mid as usize]
        && key >= arr[start as usize]
    {
        return search(arr, start as i32, mid - 1 as i32, key);
    } else if arr[mid as usize] < arr[end as usize]
        && key >= arr[mid as usize]
        && key <= arr[end as usize]
    {
...
fn main() {
    ...
    println!(
        "{}{:}",
        "Key(3) found at: ",
        find_story_id(&v1, 3).to_string()
    );
    println!(
        "{}{:}",
        "Key(6) found at: ",
        find_story_id(&v1, 6).to_string()
    );
    let v2 = vec![4, 5, 6, 1, 2, 3];
    println!(
        "{}{:}",
        "Key(3) found at: ",
        find_story_id(&v2, 3).to_string()
    );
    println!(
        "{}{:}",
        "Key(6) found at: ",
        find_story_id(&v2, 6).to_string()
    );
}
