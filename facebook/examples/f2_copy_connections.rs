...
impl CloneGraph {
    pub fn create_test_graph_directed(
        nodes_count: i32,
        edges_count: i32,
    ) -> Vec<Rc<RefCell<Node>>> {
        ...
        while i < edges_count && i < all_edges.len() as i32 {
            ...
            vertices[edge.0 as usize]
                .borrow_mut()
                .friends
                .push(vertices.get(edge.1 as usize).unwrap().clone());
            vertices[edge.1 as usize]
                .borrow_mut()
                .friends
                .push(vertices.get(edge.0 as usize).unwrap().clone());
            i = i + 1;
        }
        return vertices;
    }
...
fn clone_rec(
    root: &mut Option<Rc<RefCell<Node>>>,
    nodes_completed: &mut HashMap<i32, Rc<RefCell<Node>>>,
        ...
        .into_iter()
    {
        ...
        if !nodes_completed.contains_key(&data) {
            ...
            new_node
                .borrow_mut()
                .friends
                .push(nodes_completed.get(&data).unwrap().clone());
        }
    }
